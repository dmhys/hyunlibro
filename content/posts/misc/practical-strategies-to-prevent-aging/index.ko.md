...
references = [
    ...
    {title = "Apoptotic cell death in disease—Current understanding of the NCCD 2023", authors = "Vitale, I., Pietrocola, F., Guilbaud, E. et al.", doi = "10.1038/s41418-023-01153-w"}
]
+++
...
결론부터 말하자면, 염증 자체가 우리 몸에게 있어선 일종의 비상상황이기 때문이다. 야근, 특근과 같다. 하루 이틀은 생산성에 다소 도움이 되고, 사용하기 따라 유익할 수 있다. 그러나 사람이 1년내내 야근을 하면 지치고, 점차 생산성이 안나오게 될 것이다. 염증도 같다. 비상 상황이 지속되면 하나씩 문제가 생긴다. 현대인이 염증에 취약한건 일상적으로 염증이 일어나기 좋은 환경에 처해있기 때문이다. 어떤 것들이 우리 현대인에게 염증을 유발할까?
...
첫 번째 문제는 우리 몸에서 이송을 위해 포장될 때 발생한다. 세균은 지질다당류(LPS)를 일종의 보호막으로 가지고 있다. 따라서 우리 몸의 면역 체계는 지질다당류에 아주 강하게 반응한다. 그런데 문제는 우리가 지방을 패키징 할 때, 이 세균에서 떨어져나온 지질다당류들이 다소 섞여서 패키징되어서 배달된다는 점이다. 실제로 세균이 배달된 것은 아니지만, 면역세포의 입장에선 세균들이 대량 배달된 것과 같은 상황이다. 당연히 비상 경보가 발령된다. 이 상황은 장내 건강이 안 좋을 수록 더 자주 일어난다.
...
당연하게도 세포는 죽는다. 세포가 어떻게 죽는지는 국제 세포사 위원회(NCCD)가 이쁘게 정리해놨다.[5] 어떤 죽음이 있는지, 어떤 마커로 이들 죽음을 구분 가능한지, 어떤 질환이 있는지, 용어가이드등 다양하게 정의해놨고, 그 리스트는 다음과 같다. 
...
- Entotic cell death
- NETotic death
- Lysosome-dependent death
- Autophagy dependent death
- Immunogenic cell death

너무 복잡하고 기전이 다양하니 대부분 생략하자. 이들은 죽음의 특성에 따라 몇 가지로 나눌 수 있는데, 기초 생물학 교과서스럽게 나누자면 다음과 같이 많이 구분한다.

- 아포토시스(Apoptosis, 세포자살)
- 네크로시스(Necrosis, 괴사)
- 오토파지(Autophagy, 자가포식)
  
**아포토시스**의 경우, 깔끔한 죽음이다. 예를 들어 감염된 세포가 면역반응을 통해 자살하는 경우가 있고, 정상적으로 분화되면서도 세포가 직접 분해되는 케이스가 있다. 즉, 이는 일반적으로 프로그램된 세포의 정상 반응이며, 이때 염증은 거의 유도되지 않는다. 깔끔하다.

**네크로시스**의 경우는 그 반대다. 세포에게 극한의 손상이 가해졌을 때, 세포는 더 이상 회복하지 못한다. 파열되며 통제되지 못한(통제된 necrosis도 존재하지만 우선 무시한다.) 죽음이 일어난다. 세포 물질들이 대량으로 혈류로 쏟아져 나온다. 횡문근 융해증이 바로 이 경우에 해당한다. 당연히 몸에 아주 안좋다.

**오토파지**의 경우, 세포가 죽을 수 도 있고 안 죽을 수 도 있다. 세포의 안엔 여러가지 구성 요소들이 있다. 그 구성 요소들도 단백질이기에, 사용하다 보면 기능 저하가 일어난다. 재활용이 필요하다. 오토파지의 경우 이런 세포들을 분해해서 재활용하는 과정이며, 주로 에너지가 부족할 때 자주 일어난다. 오토파지 과정에서 잘못하면 세포가 죽는데, 이는 염증성이다. 

즉, 세포는 죽거나, 부리를 부러뜨리는 송골매처럼(낭설이지만) 오토파지를 통해 뼈를 깎아 생존을 도모할 수 있다. 그런데 사실 햄릿과는 다르게 세포에겐 죽거나 살거나 두 가지 선택지만 있지 않다. 세포는 **제 3의 선택지**를 가지고 있다. 그리고 그 선택지가 아주 큰 문제이다. 잠시 유전자 얘기를 해보자.
