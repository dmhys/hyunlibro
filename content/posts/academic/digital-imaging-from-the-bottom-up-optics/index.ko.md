...
9. [근축광학을 이제 와서 정의] 지금까지의 모든 작도 — 얇은 렌즈 공식, 주평면, 동공 — 는 $\sin\theta \approx \theta$ 라는 1차 근사 위에 서 있었다. 이름을 안 붙였을 뿐 처음부터 깔려 있었다.
10. [붕괴] $\sin\theta = \theta - \theta^3/3! + \cdots$ 에서 버린 3차항이 되돌아온 것이 수차고, 그래서 3차 수차(Seidel)가 다섯 개로 딱 떨어진다 — 다섯을 "무엇이 어떻게 깨지는가"로 한 줄씩.
11. [색수차 상세] 단색광에서도 생기는 위 다섯과 달리 색수차는 파장 축의 별개 문제 — 축상/배율 두 종류, 아베수와 저분산 유리(비싼 렌즈가 비싼 이유).
