...
아마 빛은 파동성과 입자성을 동시에 갖고있다는 얘기를 들어보았을 것이다. 사실 어느정도 널리 알려진 것에 비해 빛의 입자성은 생각보다 늦게 증명되었다. 아인슈타인이 1905년에 광양자 가설을 제안하긴 했고, 21년 노벨상 수상의 명분이 되었지만(이에 대해선 상당히 길고 정치적인 배경이 있다.), 실제론 1923년 콤프턴 산란이 증명하기 전까진 물리학자들 사이에 회의론이 꽤 있었던 것으로 알려져있다. 
...
주로 veiling flare, glare, flare등으로 부르는 현상인데, 강한 빛이 렌즈의 이상적인 경로를 따르지 않고 여러 방향으로 산란되면서 일어난다. 정면이 아닌 비스듬하게 들어오는 빛에서 종종 생기며, 그 외에는 주로 뭔가 결함때문에 생긴다. 예를들어 유분기등 렌즈의 오염(스마트폰 렌즈가 뿌옇다면 한번 닦아주자), 스크래치, 습기 등이 있다. 
...
반면 이 사진은 ghost flare, ghosting, lens ghost를 보여준다. 이건 빛이 너무 강해서 렌즈들 사이에서, 혹은 센서면에서 반사될 때 생긴다. 들어온 빛이 어디선가 반사되고, 다시 반사되면서 센서로 들어올 때 생기며, 현대에선 주로 반사 방지 코팅을 통해 잡는다. 이 두개의 설명을 보면 위의 렌즈플레어 이미지를 다시 보고 서로 다른 두개의 원인을 찾을 수 있다. 화면 좌상단에는 빛이 번져서 생긴 상이, 중앙과 왼쪽 아래쪽으로는 강한 빛이 어디선가 튕겨서 다시 돌아온 것을 볼 수 있다.
...
우선 수차에 대해 알아야 할 두가지 파라미터는 바로 동공 좌표 $\rho$와 상고 $h$ 이다. 동공 좌표 $\rho$ 는 광선이 지나는 조리개상의 위치이다. 상고 $h$는 이미지에서 상이 광축과 얼마나 떨어져있는지이다. 즉, 각각 조리개에서 광축과 떨어진 거리와 상면에서 광축과 떨어진 거리이다. 하나의 상은 조리개면의 모든 영역을 지난다. $\rho$는 '조리개를 얼마나 개방하느냐' 에 따라 바뀌는 특성이라 볼 수 있고, 상고 $h$는 '이미지의 중앙에서 얼마나 떨어졌느냐' 라는 특성으로 볼 수 있다. 아래 데모를 통해서 개념을 잡길 바란다.
...
비점수차의 경우엔 sagittal focus와 tangential focus 면이 다르게 형성되는 것을 볼 수 있다.
...
여기서 왜곡의 특성에 대해 오해를 할 수 있어서 미리 얘기해볼 것이 있다. 수차가 대략적으로 어느정도 크기다 하는 데에 사용하기엔 수차항 자체를 보는게 맞지만, 실제로 그 특성에 대해 알기 위해선 실제론 파면항의 광선위치 오차로 봐야한다. 이미 위에서 상면 만곡을 설명하기 위해선 초점면의 $\delta z$ 를 설명을 했어야했는데 전혀 설명을 하지않았다. 그래도 특성 자체를 이해하는데엔 이미지만 보면 되므로 큰 문제가 없지만, computer vision에 있어서 정말 중요한 특성이므로 조금 더 덧붙여야한다. 자세한 설명을 너무할 정도로 생략을 해보자면, 
...
여기서 단색 수차 중 왜곡의 경우 현대 렌즈설계에 있어서 가장 관대한 term이다. 왜냐하면 다른 수차의 경우 결상에 지장을 주는 반면, 왜곡의 경우엔 parallel shift라는 특성이 있기 때문에 나중에 영상처리를 통해서 거의 완벽하게 복원해낼 수 있기 때문이다. 따라서 이 term이 computer vision 관점에선 거의 유일하게 중요한 수차가 되고, 반대로 사진 촬영 측면에선 거의 중요하지 않은 term이 된다. (그리고 실제로 다소 distortion이 있어도 사진을 감상하는 데엔 별 문제가 되지않는다.)
...
색수차의 경우 축상 색수차와 배율 색수차 두개로 나뉘어진다. 위 이미지에서 축상 색수차는 축 방향으로 일어나는 색수차이기 때문에 상고와 상관없이 일어나고, 배율 색수차의 경우 상고에 영향을 받는다. 그 외에 아래 예제에서 control knob로 주어진 아베수라는 개념에 대해 짚고 넘어가자면, 아베수란 굴절력 단위당 색수차를 재는 지수이다. 굴절력이 크면 당연히 색이 더 많이 벌어질 것이기에 색수차의 품질 지표를 '단위 굴절력당 색수차'로 정한 것이다.
...
1. [리뷰: 빛] 이 챕터는 빛의 세 성질에서 출발했다 — 직진성이 "세상과 상의 1:1 매칭"이라는 카메라의 원형(바늘구멍)을 주었고, 파동성이 굴절(렌즈의 원리)과 회절(그 한계)을 주었고, 스펙트럼이 색과 색수차를 주었다.
2. [리뷰: 광학계] 바늘구멍의 어두움을 렌즈가 풀었고, 대신 초점면과 수차를 청구받았다 — 조리개는 그 사이를 오가는 다이얼이었다(광량·심도·수차·회절이 한 손잡이에 반대로 매달린 sweet spot).
3. [PSF + 가역/비가역] 이 챕터에서 만난 흐림들 — 심도, 회절, 수차 — 은 전부 "한 점이 어떤 모양으로 번져 맺히는가" 하나로 요약되고, 그 함수를 PSF라고 부른다. 렌즈라는 물건 전체가 이 명함 한 장으로 압축된다. 왜곡·비녜팅은 좌표와 밝기의 문제라 소프트웨어로 되돌릴 수 있지만, PSF로 한 번 번진 흐림은 사실상 되돌릴 수 없다 — 이 갈림길은 신호처리 챕터에서, PSF라는 언어 자체는 시뮬레이션 챕터에서 다시 만난다.
4. [미회수 ①: 비녜팅] 비녜팅의 원인은 렌즈에만 있지 않다 — 비스듬히 들어오는 빛은 그 자체로 어두워지고(cos⁴), 픽셀도 비스듬한 빛을 잘 받지 못한다. 센서 챕터에서 마저 본다.
5. [미회수 ②: 입자성] 그리고 입자성은 이 챕터에서 한 번도 쓰이지 않았다 — 빛이 알갱이로 '도착한다'는 사실은, 센서가 빛을 세기 시작하는 순간 문제가 된다(노이즈의 근원). 이것도 센서 챕터의 몫이다.
6. [클로징] 다음 챕터는 센서 — 초점면까지 도착시킨 빛이 숫자가 되는 과정이다.
