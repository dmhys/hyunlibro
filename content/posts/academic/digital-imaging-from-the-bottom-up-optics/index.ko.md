...
이와 반대되는 개념으로는 색수차가 있다. 굴절률은 파장과 반비례 관계이기 때문에, 빛의 파장이 짧아질수록 굴절이 많이 된다. 같은 각도로, 같은곳으로 렌즈면에 진입하더라도 빛의 파장에 따라 경로가 다르다. 위의 개념도에서 적색파장(긴 파장)은 덜 휘고, 보라색에 가까운 파란 파장(짧은 파장)은 더 많이 휜 것을 볼 수 있다. 실제 사진 예시는 아래와 같다.
...
용어가 완전히 정착하진 않은 분야인데, 원인은 두개가 있어서 여기서 용어를 좀 다듬으면서 설명해보겠다. 위 이미지에는 두개의 상이 둘 다 들어있다. 둘 다 강한 빛이 들어올때 광학적으로 올바른 경로로 들어오는 빛이 아닌, 다른 잡광들이 유입되면서 생기는 현상이다.
...
주로 veiling flare, glare, flare등으로 부르는 현상은 빛이 번져서 일어나는 현상이다. 강한 빛이 옆에서 들어오거나, 앞에서 들어와도 빛이 번질 때 일어나는 현상이다. 사진가들이 가지고 다니는 카메라에서 렌즈 앞에 있는 것을 후드라고 하는데, 이 빛이 옆에 들어와서 번지는것을 막기 위해서 사용한다. 다만 일반적으로 렌즈는 빛이 강하게 번지지 않으니, 만일 내 핸드폰에 사진이 이렇게 찍힌다면 렌즈를 한번 닦아주자. 주로 손 유분기로 인해서 생긴다.
...
서사 재구성 — 분류가 아니라 **사슬**로. 해법이 다음 청구서를 낳고, 그 청구서가 다음 해법을 부른다.
그리고 마지막에 고리로 닫힌다. (0~2는 본문에 이미 있음. 3부터가 새로 쓸 부분)

**0. 바늘구멍** — 완벽한 원근투영에 무한 심도. 문제는 어둡다는 것. ※ 원리적으로 흐리기도 하다 — $d_{opt}=\sqrt{2.44\lambda L}$, 최적점에서도 블러가 4µm 픽셀 92개 폭. 바늘구멍 절에 회수 재료

**1. 렌즈로 구멍을 키웠다** → 광량을 얻었다. 청구서가 셋 동시에 온다.
- 초점면이 생겼다 → **심도·보케** (본문 있음)
- 구면이라 한 점에 못 모은다 → **수차** (본문 있음)
- 유리가 파장마다 다르게 꺾는다 → **색수차** (본문 있음)

**2. 수차를 줄이려고 조리개를 조였다**
- ※ 역사적으로 stop은 광량이 아니라 **수차 제어용**으로 발명되었다 — 조리개 절(282줄)에 이미 써둔 사실이 여기서 회수된다
- 얻음: 구면수차가 $\rho^3$으로 급감, 심도도 덤으로
- 청구서: **회절**. 이게 sweet spot의 정체 (본문 있음)

**3. 축외 화질을 잡으려고 조리개를 렌즈에서 떼어 놓았다**
- 조리개 위치는 코마·비점수차·상면만곡을 조절하는 레버다 — 초기 풍경 렌즈가 조리개를 앞에 둔 이유
- 청구서: **왜곡**. stop-shift가 구면의 비선형성을 왜곡으로 옮긴다. 조리개가 렌즈에 붙어 있을 땐 주광선이 화각과 무관하게 늘 렌즈 중심을 지나 왜곡이 0이었다
- [수치] 렌즈 한 장, 조리개만 앞뒤 70mm → 배럴 −4%에서 핀쿠션 +14%

**4. 왜곡을 없애려고 조리개 기준으로 대칭 배치했다** (Rapid Rectilinear, 1866)
- 얻음: 대칭에 대해 부호가 뒤집히는 항들이라 **왜곡·코마·배율색수차가 한꺼번에** 상쇄된다. rectilinear라는 이름의 유래
- 청구서 ①: 유리가 여러 장 필요해진다 → 계면마다 4% 반사 → **플레어·고스트**
- 청구서 ②: 대칭을 못 쓰는 설계에선 왜곡이 돌아온다 — SLR 미러 공간 때문에 강제된 레트로포커스 광각이 배럴이 심한 이유

**5. 색수차를 잡으려고 성질이 다른 유리를 붙였다** (아크로매트, 크라운+플린트)
- 청구서: 유리 장수가 또 늘고, 계면이 또 는다 → 4번의 청구서 ①이 누적된다

**6. 반사를 줄이려고 코팅을 했다** (AR 코팅, 1935)
- 얻음: 4% → 0.5% 이하. **이때 비로소 10매 이상의 설계가 가능해졌다** — 코팅 이전엔 매수 자체가 제약이었다
- 원리: 박막 앞뒷면 반사의 상쇄 = **간섭**. 에어리 패턴의 고리와 같은 현상이다. 이 글은 간섭을 이름 없이 두 번 썼고, 여기서 이름을 준다
- 청구서: 코팅은 파장·입사각 의존이라 잔여 반사가 남는다. 렌즈 앞이 보라·녹색으로 번들거리는 게 그 증거

**7. 광량을 더 얻으려고 구경을 키웠다**
- 청구서 ①: 테두리와 경통이 축외 광속을 자른다 → **비녜팅**. 고양이눈 보케가 눈에 보이는 증거
- 청구서 ②: 수차가 $\rho^3$으로 돌아온다 → **2번으로 되돌아간다**

**닫기 — 사슬이 아니라 고리였다**
- 조이면 회절, 열면 수차. 유리를 늘리면 반사, 줄이면 수차. 대칭이면 왜곡이 잡히지만 화각이 묶인다
- 제목의 진짜 뜻: 렌즈가 나빠서 이상적일 수 없는 게 아니라, **모든 해법이 다음 청구서를 낳아 완결된 해가 존재하지 않는다**는 것. 렌즈 설계는 해결이 아니라 배분이다
- ※ 자연 비녜팅 cos⁴은 이 고리 밖 — 유리와 무관한 순수 기하 (정리 절 / 센서 챕터)

**정리 절로 넘길 것 — 고리를 다 돌고 나서 다시 자른다**
- 되돌릴 수 있는가로 자르면 세 층: **좌표**(왜곡 → 좌표 변환), **밝기**(비녜팅 → 게인 맵), **선명함**(수차·회절 → PSF 디컨볼루션, 노이즈 증폭)
- 잡광만 예외다. 정보가 **더해진** 것이라 되돌릴 수 없다
- 즉 서사는 사슬로 끌고 가고, 정리에서 분류로 닫는다
