...
특히 Essential matrix의 최소해는 5쌍의 point대응인데, 6자유도에서 scale term이 빠지기 때문에 5dof이기 떄문이다. 이를 우리는 5-point algorithm이라 부른다. 어쩌다 인터넷에서 8-point가 느리지만 더 정확한 결과를 생성하는 장점이 있다는 글을 봤다. **아니다.** 경험적으로는 전혀 그렇지 않았다. 첫째, RANSAC을 돌려야 하는 이상, 8과 5는 큰 차이가 있다. 둘째, 5-point algorithm이 완벽하게 수학적으로 모델을 정의하는 이상, 8-point는 이미 내재적으로 오차가 있다. Redundant constraint이기 때문이다. 숫자를 많이 쓰는 것과 숫자를 잘 쓰는 것은 다르다. Redundant constraint라고 더 정확한가? 이미 여러 라이브러리들에선 5-point가 defacto standard다.

다만 이 5point algorithm은 - Hongdong li는 아주아주 쉽다고 claim하는 것과 다르게 - 상당히 어려워서 많이 쓰이는 빈도에 비해서 도저히 이해가 힘들었다. 그야 polynomial까지의 유도는 그렇게 어렵지 않은데, 그 다음에 그 다항식의 해를 구하는 과정에서 정말 생소한 수학적 개념이 많이 나온다. 예전에 직접 구현하려 했으나, 그뢰브너 기저등 이런 기저에 깔린 생소한 개념에 박살난 채 도망갈 수 밖에 없었다. (대충 거의 구현이 된거 같았는데, 되는 척 하다가도 죽어라 안되었다.)
...
$t$는 조금 편한데, 직관적으로 봐서 애초에 singular value에 0이 생긴 이유가 translation방향으로 degenerate했기 때문이다. 즉, SVD의 결과물인 $U$의 left singular vector중 0에 해당되는, 즉 diagonal matrix가 정렬되었다는 가정하에 세 번째 column이 바로 0에 해당하는 left singular vector가 된다. 또, degenerate할 때 축의 방향은 어느쪽이던 상관없다. 즉,
...
딱히 기하적으로 풀 방법은 없고, point의 R T를 이용해서 삼각 측량 했을 때, 양쪽 이미지에서 전부 depth가 양수가 되도록 하면 된다. 예를 들어 colmap같은 경우는 주어진 point를 전부 depth calculation해서, 양 쪽 이미지에서 depth가 모두 양수이며 `max_depth` 이하인 point가 있는지 없는지를 검사해서, 있으면 valid configuration이라고 본다.
...
왜 3x3 matrix에서 singular value 두개가 같고, 하나는 0인 것이 Essential matrix의 필요충분조건인가?

skew-symmetric matrix-> 공간에 대해 크로스곱-> 즉 그 축에 대해 변하지 않는 벡터는 그 축 자체이며, 0이됨.'
또 trace가 0이니까 남은 두 eigenvalue의 합은 0이 되어야함. 공간에 대해 cross product를 하면 전부 회전을 할 수 밖에 없고, 유지되는 eigenvector는 축 하나밖에 없음. 즉 real eigenvalue가 나타날 수 없음.

canonical form of skew symmetric matrix란 무엇인가?
...
2편 Nistér's algorithm과 Gröbner basis
3편 Hongdong Li's Hidden variable method
4편 구현체들 살펴보기

opencv
Nister+ Durand-Kerner method
colmap
- Nister's
- FindPolynomialRootsCompanionMatrix(Durand-Kerner method)
