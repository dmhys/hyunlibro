...
필드에 있으면서 사실 영상기하를 하는 사람들이, 카메라가 어떻게 작동하는지에 대한 정말 vague한 이해를 가지는 경우를 많이 봤다. 시대가 나아가서 신경망의 시대가 되며 이는 더 심해졌고, 모든 이미지는 H W 3 tensor 데이터 하나로 고정되어버렸다. 

근본주의적인 면에서 더 나은 이해를 모두가 함양해야한다는 살짝 꼰대같은 생각을 하고있긴했다. 그런데 시대가 physical ai가 되면서, simulation에 대한 관심이 많아졌다. 이제는 진짜 좀 쓸만한 시기가 된거같기도하다는 생각에, 이렇게 글을 써본다.

앞 세 챕터의 경우엔 조금 더 카메라에 초점을 맞추며, 시계열상 정렬되어있는 일들을 순서대로 가져간다. 여기서 대략적으로 센서에 대한 이해와 함께 뒷 채널에서 설명할 내용들을 쉽게 이해하기 위한 상식이 된다.

광학에 대한 이해에선 빛이 어떻게 해서 센서에 닿는지.
그 다음 센서에 도달한 빛이 어떻게 전기신호로 변환되는지
이렇게 샘플링한 데이터가 어떻게 RGB 이미지로 바뀌는지.

반면 뒤 네 챕터는 조금 더 로보틱스, 3D vision도메인에서 끌고가본다. 센서 sampling이란 측면에서 시공간에 대해 무엇을 알아야하는지, 그리고 depth camera는 어떤 특징이 있는지. 이렇게 얻은 특징에 대한 개념을 바탕으로 lidar 에 대해 이해를 가져가본다.

후반부 네 챕터의 마지막은 realistic simulation을 위해 센서 특징이 어떻게 되는지 살펴보며, 추가적으로 imu의 noise model또한 광학계는 아니지만 상당히 많이 사용하는 센서이니만큼 살짝 담아보았다.

===

# 광학에 대한 이해

광학계란 빛의 영역. 즉, 외부에서 나온 빛이 어떻게 해서 센서까지 닿게 되는지, 그 사이 어떤 요소들을 지나는지에 대한 내용
